...
bool ReadModuleInfoFromELF( const wxString& file, const ModuleInfo::DependencySet& dependencies, ModuleInfo& info )
{
    ...
    wxLogError("      magic type//ei_class:  %0x", elf_file_header.e_ident[EI_CLASS]);
    wxLogError("      magic type//ei_data:   %0x", elf_file_header.e_ident[EI_DATA]);
    wxLogError("      magic type//ei_osabi:  %0x", elf_file_header.e_ident[EI_OSABI]);
    wxLogError("      magic type//ei_abiver: %0x", elf_file_header.e_ident[EI_ABIVERSION]);
    wxLogError("      magic type//e_machine: %0x", elf_file_header.e_machine );
...
#error No support for other executable formats is implemented.
#endif
        wxLogError("      owntype: %0lx", own_info.type_magic );
        wxLogError("      pi type: %0lx", pi_info.type_magic );
        wxLogError("      diff:    %0lx", own_info.type_magic ^ pi_info.type_magic);
...
{
    DataStreamEvent event( wxEVT_OCPN_DATASTREAM, 0 );
...
{
    if (! g_options) return NULL;

    size_t parentid;
    switch (parent) {
    case PI_OPTIONS_PARENT_DISPLAY:
        parentid = g_options->m_pageDisplay;
    break;
    case PI_OPTIONS_PARENT_CONNECTIONS:
        parentid = g_options->m_pageConnections;
    break;
    case PI_OPTIONS_PARENT_CHARTS:
        parentid = g_options->m_pageCharts;
    break;
    case PI_OPTIONS_PARENT_SHIPS:
        parentid = g_options->m_pageShips;
    break;
    case PI_OPTIONS_PARENT_UI:
        parentid = g_options->m_pageUI;
    break;
    case PI_OPTIONS_PARENT_PLUGINS:
        parentid = g_options->m_pagePlugins;
    break;
    default:
    ...
    return g_options->AddPage( parentid, title );
}

bool DeleteOptionsPage( wxScrolledWindow* page )
{
    if (! g_options) return false;
    return g_options->DeletePluginPage( page );
}
